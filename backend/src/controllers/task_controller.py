...
# def get_tasks(jwt_payload):
def get_tasks():
  try:
    tasks = Task.query.order_by(Task.phase_id, Task.order).all()
...
# @requires_auth('get:tasks_per_phase')
# def get_tasks_per_phase(jwt_payload, phase_id):
def get_tasks_per_phase(phase_id):
  try:
    tasks = Task.query.filter(Task.phase_id == phase_id).order_by(Task.order).all()
